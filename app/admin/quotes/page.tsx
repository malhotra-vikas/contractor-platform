...
import { Building2, FileText, Search, Eye, Clock, CheckCircle, XCircle, DollarSign } from "lucide-react"
...
export default function AdminQuotesPage() {
    ...
    const mockQuotes: QuoteRequest[] = [
      {
        ...
        subcontractor_company: "ABC Construction",
        status: "pending",
      ...
      },
      {
        ...
        subcontractor_name: "Mike Johnson",
        subcontractor_company: "Johnson Contractors",
        status: "approved",
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (quote) =>
          ...
          quote.subcontractor_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          quote.subcontractor_company.toLowerCase().includes(searchTerm.toLowerCase()),
      )
    }
  ...
  const getPriorityLevel = (requestedDate: string) => {
    const today = new Date()
    const requestDate = new Date(requestedDate)
    const daysUntilDelivery = Math.ceil((requestDate.getTime() - today.getTime()) / (1000 * 3600 * 24))
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Review and respond to quote requests from subcontractors</p>
        </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    ...
                    {quotes
                      .filter((q) => q.quote)
                      .reduce((sum, q) => sum + (q.quote?.total_amount || 0), 0)
                      .toLocaleString()}
                  </p>
                </div>
        ...
        {filteredQuotes.length === 0 ? (
          ...
          <div className="space-y-6">
            {filteredQuotes.map((quote) => {
              const priority = getPriorityLevel(quote.requested_delivery_date)
              return (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <CardDescription>
                            {quote.subcontractor_name} • {quote.subcontractor_company}
                          </CardDescription>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {priority === "high" && <Badge variant="destructive">Urgent</Badge>}
                        {priority === "medium" && <Badge variant="secondary">Medium</Badge>}
